...
* **Phần cứng:** Board mạch nhúng (VD: Raspberry Pi, BeagleBone Black...), Thẻ nhớ SD/MicroSD, Cáp kết nối Serial/UART.
* **Phần mềm/Công cụ:**
  * Host PC chạy hệ điều hành Linux (Ubuntu/Debian).
  * Cross-compiler toolchain (VD: `arm-linux-gnueabihf-`).
  * Mã nguồn BusyBox (phiên bản ổn định mới nhất).
  * Mã nguồn/Image của Linux Kernel đã được biên dịch (`zImage` hoặc `uImage`).
...
Trong giao diện dòng lệnh của Bootloader (thường là U-Boot), tiến hành thiết lập biến `bootargs`. Cú pháp ví dụ:
```bash
